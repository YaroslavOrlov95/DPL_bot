...
def company_name(message, a):
	...
	h = message.contact.phone_number
	i = re.sub('\+', '', h)
	number = i
...
def chas_yur(message, first_name, username, number, a):
	...
	if message.text == 'Частное лицо':
		...
		bot.send_message(user_id, f'Ваше имя записанно как *{first_name}*\n\nВаш номер записан как *{number}\n\n{company_name}*', reply_markup=kb, parse_mode="Markdown")
		bot.register_next_step_handler(message, send_welcome, a)
...
def acc(message):
	...
	if message.text == '👈 В меню':
		...
		for i in user_orders:
			...
			item_add = types.InlineKeyboardButton(text='Добавить информацию к заказу', callback_data='add', reply_markup=kb)
			markup_inline.add(item_add)
			bot.send_message(user_id, f'Ваш user id: *{i[2]}*\nid Вашего заказа: *{i[1]}*\nНомер телефона: *{i[3]}*\nТехническое задаие: *{i[4]}*\nСтатус Вашего заказа: *{i[5]}*\nДата создания заказа: *{i[6]}*\n', reply_markup=markup_inline, parse_mode="Markdown")
			bot.register_next_step_handler(message, send_welcome, a)
